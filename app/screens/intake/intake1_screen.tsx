...
export default function IntakeOneScreen({navigation, route}: IntakeOneScreenProps) {
 const {formState, setFieldValue, handleGenderSelect, errors, validateField, validateForm, handleSubmit, isEdit, patientNumber} = useIntakeForm(route.params?.formData);
 ...
 const handleGenderDone = (item: Gender[]) => {
  ...
  const handleNext = async () => {
     const response = await handleSubmit(patientNumber);
     if(response){
      navigation.navigate("IntakeTwoScreen", {formData: formState, isEdit: isEdit, patientNumber: patientNumber});
     }
 };
